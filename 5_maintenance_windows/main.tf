/*
  CSA - SSM Demo - Maintenance Windows
*/

# Get IAM role for managed services created in step 4
data "aws_iam_role" "ssm-patching-role" {
  name = "ssm-patching-role"
}

# Get the SSM Document created in step 4
data "aws_ssm_document" "ssm-document" {
  name = "PatchServers"
}
...
resource "aws_ssm_maintenance_window_task" "test-window-task" {
  ...
  max_errors       = 100
  priority         = 1
  service_role_arn = data.aws_iam_role.ssm-patching-role.arn
  task_arn         = data.aws_ssm_document.ssm-document.arn
